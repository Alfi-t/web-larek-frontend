...
#### Класс Page
Наследует базовый функционал от класса Component и добавляет методы и свойства для управления основными элементами страницы, такими как счётчик корзины, каталог товаров и состояние блокировки.

**Свойства:**

- counter: HTMLElement — элемент для отображения счётчика корзины.
- catalog: HTMLElement — элемент галереи каталога товаров.
- wrapper: HTMLElement — элемент-обёртка страницы.
- basket: HTMLElement — элемент корзины для управления событиями.
- events: IEvents — объект для работы с пользовательскими событиями.

**Методы:**

- set counter(value: number) Обновляет текстовое содержимое счётчика корзины.
- set catalog(items: HTMLElement[]) Заменяет содержимое элемента каталога на переданные элементы.
